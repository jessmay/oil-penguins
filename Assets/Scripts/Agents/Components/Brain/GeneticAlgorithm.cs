...
public class GeneticAlgorithm : MonoBehaviour {
	...
	public const int TARGETS_PER_GENOME = 5;
	...
	void Start ()
	{
		...
		if(Options.populationName == null){
			...
			Type genome = typeof(FiveFeelerGenome);
			populationSize = 50;
	...
	void FixedUpdate ()
	{
		...
		if (++tick == TICKS_PER_GENOME()) {
			...
			//Move to next target.
			currTarget = (currTarget+1) % TARGETS_PER_GENOME;
			...
			if(currTarget == 0) {
				...
				population[populationIndex].fitness = population[populationIndex].totalFitness/TARGETS_PER_GENOME;///(testSubject.targetsEnabled?testSubject.totalTargets():1);
